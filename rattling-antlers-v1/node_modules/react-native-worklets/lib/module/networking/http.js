...
const FORBIDDEN_REQUEST_HEADERS = new Set(['accept-charset', 'accept-encoding', 'access-control-request-headers', 'access-control-request-method', 'connection', 'content-length', 'cookie', 'cookie2', 'date', 'dnt', 'expect', 'host', 'keep-alive', 'origin', 'referer', 'set-cookie', 'te', 'trailer', 'transfer-encoding', 'upgrade', 'via']);
...
export function normalizeMethod(method) {
  ...
  return NORMALIZED_METHODS.includes(uppercasedMethod) ? uppercasedMethod : method;
}
...
export function isSupportedRequestUrl(url) {
  const scheme = ABSOLUTE_URL_SCHEME_PATTERN.exec(url)?.[1];
  return scheme !== undefined && SUPPORTED_SCHEMES.includes(scheme.toLowerCase());
}
...
export function isForbiddenRequestHeader(name, value) {
  ...
  if (FORBIDDEN_REQUEST_HEADERS.has(lowerCaseName) || FORBIDDEN_REQUEST_HEADER_PREFIXES.some(prefix => lowerCaseName.startsWith(prefix))) {
    return true;
  }
  return METHOD_OVERRIDE_HEADERS.has(lowerCaseName) && value.split(',').some(method => isForbiddenMethod(method.trim()));
}
export function combineHeader(headers, name, value) {
  const lowerCaseName = name.toLowerCase();
  const existing = headers.find(([headerName]) => headerName.toLowerCase() === lowerCaseName);
...
export function hasHeader(headers, name) {
  const lowerCaseName = name.toLowerCase();
  return headers.some(([headerName]) => headerName.toLowerCase() === lowerCaseName);
}
export function getHeader(headers, name) {
  ...
  const values = headers.filter(([headerName]) => headerName.toLowerCase() === lowerCaseName).map(([, value]) => value);
  return values.length > 0 ? values.join(', ') : null;
}
export function sortAndCombineHeaders(headers) {
  ...
  return Array.from(combined.keys()).sort((left, right) => {
...
export function extractCharset(mimeType) {
  if (mimeType === null) {
    return undefined;
  }
